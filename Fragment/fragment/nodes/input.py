...
class Input(Node):
    ...
    def mouse_position(self):
        return Vec2(self._window.win.get_pointer(0).get_x(), self._window.win.get_pointer(0).get_y())


    @mouse_position.setter
    def mouse_position(self, position):
        self._window.win.move_pointer(0, int(position[0]), int(position[1]))
    ...
    def mouse_visible(self, visible):
        ...
        self._window.win.request_properties(props)

    def _update(self, task):
        self.input_map = self.input_map
        for key in self.inverse_input_map.keys():
            k = ButtonHandle(key)
            if self._window.mouseWatcherNode.is_button_down(k):
